...
from keras.layers import Conv3D, MaxPooling3D, UpSampling3D, GlobalAveragePooling3D, Input
...
TEST_FILEPATH = "stage1_a3d/0a27d19c6ec397661b09f7d5998e0b14.a3d"

SUBJECT_LABELS_FILE = "labels/stage1_labels.csv"
...
def get_model():
    ...
    model.add(MaxPooling3D(pool_size=(2, 2, 1), input_shape=INPUT_SHAPE))
    #add stride? 1, see paper
#    model.add(Conv3D(32, kernel_size=(3, 3, 3), padding='same',
...
sae_checkpoint = ModelCheckpoint("weights/best_sae.h5", monitor='loss', verbose=1, save_best_only=True, mode='min')
vgg_checkpoint = ModelCheckpoint("weights/best_sae.h5", monitor='loss', verbose=1, save_best_only=True, mode='min')

#log = get_model().fit_generator(generator=generator(), steps_per_epoch = 200, epochs = 1000, verbose=2, callbacks=[saecheckpoint])
