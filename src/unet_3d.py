...
from keras.layers import Conv3D, MaxPooling3D, UpSampling3D, Activation, ZeroPadding3D, Reshape
...
def unet_model_3d(input_shape, downsize_filters_factor=1, pool_size=(2, 2, 2), n_labels=1,
                  initial_learning_rate=0.1, deconvolution=False):
    """
    ...
    print (conv2.shape)
    print (up6.shape)
    #up6 = ZeroPadding3D((0,0,1))(up6) 
    #up6 = Reshape((None, 256, 128, 165, 128))(up6)
    ...
    up7 = get_upconv(pool_size=pool_size, deconvolution=deconvolution, depth=0,
                     nb_filters=int(64/downsize_filters_factor), image_shape=input_shape[-3:])(conv6)
    ...
    #model.compile(optimizer=Adam(lr=initial_learning_rate), loss=dice_coef_loss, metrics=[dice_coef, 'acc'])
    model.compile(optimizer=SGD(lr=initial_learning_rate, decay=0.005), loss=dice_coef_loss, metrics=[dice_coef, 'acc'])
