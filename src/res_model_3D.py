...
def build_model(input_shape):
	...
	#x1_merged = merge([x1, x1_ident],mode='concat', concat_axis=4)
	...
	#x2_merged = merge([x2_1,x2_ident],mode='concat', concat_axis=4)
	...
	#x3_merged = merge([x3_1,x3_ident],mode='concat', concat_axis=4)
	...
	#x4_merged = merge([x4_1,x4_ident],mode='concat', concat_axis=4)
	...
	#xout_threat = dense_branch(xpool_norm,name='o_threat', outsize=1, activation='sigmoid')
	...
	#model = Model(input=xin,output=xout_threat)
	...
	model.compile(optimizer=opt,loss='binary_crossentropy', metrics=['accuracy'], loss_weights={'o_d':0.0, 'o_lob':0.0, 'o_spic':0.0, 'o_mal':1.0, 'o_fp':0.0})	
	print ('compiled')
